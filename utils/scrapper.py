...
class ScrapperClient:
    ...
    def get_tweets(self, query, count):
        ...
        for item in tweepy.Cursor(self.api.search_tweets, q="{q} -filter:retweets".format(q=query), tweet_mode="extended", count=count, lang="id").items():
            if len(self.tweets) == count:
                break
            self.tweets.append([item.id, item.created_at.strftime("%d-%b-%Y (%H:%M:%S)"), item.full_text.replace("\n", " ")])
